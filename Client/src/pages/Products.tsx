...
function Products({ cartOpen, setCartOpen, isAuthenticated, onMyAccountClick, onLogout, userId = 'guest', cart, setCart, cartLoading }: ProductsProps) {
  ...
  // Save cart to DB on change with debounce
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', maxWidth: '1400px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '30px' }}>
            <button
              ...
              style={{ background: 'transparent', border: 'none', color: 'white', fontSize: '14px', letterSpacing: '1px', cursor: 'pointer', textTransform: 'uppercase', padding: 0, transition: 'color 0.2s' }}
              onMouseEnter={e => e.currentTarget.style.color = '#d4af37'}
              onMouseLeave={e => e.currentTarget.style.color = 'white'}
            >
              Cart {cartTotal > 0 && (
                <span style={{ background: '#d4af37', color: '#5c1a33', borderRadius: '50%', padding: '1px 6px', fontSize: '11px', marginLeft: '5px', fontWeight: '700' }}>{cartTotal}</span>
              )}
            </button>
            ...
            <div style={{ position: 'relative' }}>
              <button
                ...
                style={{ background: 'transparent', border: '1px solid rgba(212,175,55,0.6)', color: '#d4af37', fontSize: '14px', letterSpacing: '1px', cursor: 'pointer', textTransform: 'uppercase', padding: '8px 18px', borderRadius: '4px', transition: 'all 0.2s' }}
                onMouseEnter={e => { e.currentTarget.style.background = '#d4af37'; e.currentTarget.style.color = '#5c1a33' }}
              ...
              {isAuthenticated && profileOpen && (
                <div style={{ position: 'absolute', top: 'calc(100% + 10px)', right: 0, background: 'white', borderRadius: '6px', minWidth: '200px', boxShadow: '0 8px 30px rgba(0,0,0,0.15)', border: '1px solid #e8e8e8', overflow: 'hidden', animation: 'modalSlideIn 0.2s ease-out' }}>
                  <div style={{ padding: '16px 20px', borderBottom: '1px solid #f0f0f0', background: '#faf7f5', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                    ...
                    <button onClick={() => setProfileOpen(false)} style={{ background: 'transparent', border: 'none', color: '#bbb', fontSize: '16px', cursor: 'pointer', lineHeight: 1, padding: 0 }}>✕</button>
                  </div>

                  {authService.getCurrentUser()?.role === 'Admin' && (
                    <button
                      onClick={() => { navigate('/admin'); setProfileOpen(false) }}
                      style={{ width: '100%', padding: '14px 20px', background: 'transparent', border: 'none', color: '#5c1a33', fontSize: '14px', letterSpacing: '1px', textAlign: 'left', cursor: 'pointer', textTransform: 'uppercase', transition: 'background 0.2s', borderBottom: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', gap: '8px' }}
                  ...
                  <button
                    ...
                    style={{ width: '100%', padding: '14px 20px', background: 'transparent', border: 'none', color: '#7d2e54', fontSize: '14px', letterSpacing: '1px', textAlign: 'left', cursor: 'pointer', textTransform: 'uppercase', transition: 'background 0.2s' }}
        ...
        <div style={{ flex: 1, padding: '30px' }}>
          <div style={{ marginBottom: '30px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <input type="text" placeholder="Search products..." value={search} onChange={e => setSearch(e.target.value)} style={{ padding: '10px 16px', border: '1px solid #e8e8e8', borderRadius: '4px', fontSize: '14px', width: '220px', outline: 'none' }} />
              <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ padding: '10px 14px', border: '1px solid #e8e8e8', borderRadius: '4px', fontSize: '14px', color: '#555', cursor: 'pointer', outline: 'none' }}>
              ...
              <div key={product._id} style={{ border: '1px solid #e8e8e8', background: 'white', borderRadius: '4px', overflow: 'hidden', transition: 'box-shadow 0.3s, transform 0.2s', cursor: 'pointer' }}
                ...
                <div style={{ padding: '16px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <input type="number" min={1} max={product.quantityAvailable} value={getQty(product._id)}
      ...
      {selectedProduct && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setSelectedProduct(null)}>
          <div style={{ background: 'white', borderRadius: '8px', width: '480px', maxWidth: '90vw', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '28px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <input type="number" min={1} max={selectedProduct.quantityAvailable} value={getQty(selectedProduct._id)}
                  ...
                  style={{ width: '70px', padding: '10px', fontSize: '14px' }} onClick={e => e.stopPropagation()} />
      ...
      {cartOpen && (
        <div style={{ position: 'fixed', top: 0, right: 0, width: '360px', height: '100vh', background: 'white', boxShadow: '-4px 0 20px rgba(0,0,0,0.15)', zIndex: 200, display: 'flex', flexDirection: 'column' }}>
          ...
          {/* Date pickers inside cart */}
          {cart.length > 0 && (
            <div style={{ padding: '16px 20px', borderTop: '1px solid #f0f0f0', background: '#faf7f5' }}>
              <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: '10px', color: '#aaa', letterSpacing: '1px', textTransform: 'uppercase', display: 'block', marginBottom: '4px' }}>Start Date</label>
                ...
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: '10px', color: '#aaa', letterSpacing: '1px', textTransform: 'uppercase', display: 'block', marginBottom: '4px' }}>End Date</label>
              ...
              <button
                ...
                style={{ width: '100%', padding: '14px', background: '#5c1a33', color: 'white', border: 'none', fontSize: '14px', letterSpacing: '1px', cursor: 'pointer' }}
              >
                Proceed to Checkout
              </button>
            </div>
      ...
      {/* Toast */}
      {toast && (
        <div style={{ position: 'fixed', bottom: '30px', left: '50%', transform: 'translateX(-50%)', background: '#2d6a4f', color: 'white', padding: '12px 24px', borderRadius: '6px', fontSize: '14px', fontWeight: '500', zIndex: 999, boxShadow: '0 4px 20px rgba(0,0,0,0.2)' }}>
          {toast}
        </div>
      )}
    </div>
